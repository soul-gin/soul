...
采用数组实现，没有首尾指针(首尾相连的数组)

对比ConcurrentLinkedQueue，用数组实现的速度更快(但是JDK中没有基于Array数组实现的Queue; 数组扩容需要拷贝)

> 如何判断新增数据应该加在哪个位置?顺序个数直接对容量取模, 然后跟进策略进行覆盖或其他操作(等待策略)
>
> 假如长度为8，当添加到第12个元素的时候在哪个序号上呢？用12%8决定
>
> 当Buffer被填满的时候到底是覆盖还是等待，由Producer决定
>
> 长度设为2的n次幂，利于二进制计算，例如(取模操作由位运算替换, 提高效率)：12%8 = 12 & (8 - 1)  pos = num & (size -1)
